...
class BrandAnalyzer:
    ...
    KNOWN_BRAND_SIGNATURES: Dict[str, Dict[str, Any]] = {
        "proUSB_V9": {
            "magic": "C92B20B7",
            "payload_size": 16,
            "dll_names": ["V9RFL.dll"],
            "card_types": {
                "guest": {"type_byte_high": 0x6},
                "master": {"type_byte_high": 0xB},
                "building": {"type_byte_high": 0xC},
                "floor": {"type_byte_high": 0xD},
                "emergency": {"type_byte_high": 0xA},
                "auth": {"type_byte_high": 0x0},
                "record": {"type_byte_high": 0x8},
            },
            "checksum_algos": ["byte15_fb", "sum14_zero_byte15"],
        },
    }
    ...
    def _match_known_brand(self, magic: str, payload_size: int,
        ...
        # 先按品牌名精准匹配
        if brand_hint:
            for key in self.KNOWN_BRAND_SIGNATURES:
        ...
        if dll_names:
            ...
            for key, sig in self.KNOWN_BRAND_SIGNATURES.items():
